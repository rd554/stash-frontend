...
export default function AIFinancialInsights({ user, onGetTips, refreshTrigger }: AIFinancialInsightsProps) {
  ...
  const getFallbackInsights = useCallback((): Insight[] => {
    if (!user) return []
  ...
  }, [user, refreshTrigger, loadInsights])
  ...
  const getInsightTextColor = (): string => {
    ...
    try {
      if (insight.insightId) {
        ...
        if (response.success && response.data) {
          ...
          interface ChatbotContextResponse {
            sessionId: string
            suggestedQuestion: string
            context: string
          }
          
          const contextData = response.data as ChatbotContextResponse
          
          // Store context data for chatbot
          localStorage.setItem('stash-ai-chatbot-context', JSON.stringify({
            sessionId: contextData.sessionId || '',
            suggestedQuestion: contextData.suggestedQuestion || '',
